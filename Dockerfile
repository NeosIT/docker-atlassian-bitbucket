...
RUN set -x \
    && apt-get update \
    && apt-get install -y --no-install-recommends ttf-dejavu git curl procps \
    ...
    && mkdir -p               "${BITBUCKET_INSTALL}/elasticsearch/logs" \
    && chown -R daemon:daemon "${BITBUCKET_INSTALL}/elasticsearch/logs" \
